...
describe('Endpoint: /hba-rule', () => {
    it('should list rules on GET', async () => {
        const rules = await request(expressApp).get('/hba-rule').expect(200);
    ...
    it('should create new rule on POST and be able to retrieve it with GET + id', async () => {
        const ruleToCreate = <HbaRuleModel>{
            type: 'host',
            address: '127.0.0.1',
            authMethod: 'md5',
            databases: ['postgres'],
        ...
        const createRuleResponse = await request(expressApp).post('/hba-rule').send(ruleToCreate).expect(201);

        expect(createRuleResponse.body).toBeDefined();
        expect(createRuleResponse.body.id).toBeDefined();

        const getRuleResponse = await request(expressApp).get(`/hba-rule/${createRuleResponse.body.id}`).expect(200);

        const createdRule = getRuleResponse.body as HbaRuleModel;
    ...
    it('should create new rule on POST, change it on PUT and retrieve it with GET + id', async () => {
        ...
        const ruleToUpdate = <HbaRuleModel>{
        ...
        await request(expressApp).put(`/hba-rule`).send(ruleToUpdate).expect(200);

        const getRuleResponse = await request(expressApp).get(`/hba-rule/${ruleToUpdate.id}`).expect(200);

        const updatedRule = getRuleResponse.body as HbaRuleModel;
    ...
    it('should create and delete a rule on through POST and DELETE', async () => {
        ...
        await request(expressApp).delete(`/hba-rule/${createRuleResponse.body.id}`).expect(200);

        await request(expressApp).get(`/hba-rule/${createRuleResponse.body.id}`).expect(404);
    });
});
